...
---
  ...
  {%- for event in site.data.bhf-events-2025 -%} 
     <img src="images/{{event.ImageName}}" alt="{{event.ImageDesc}} | Credit {{event.ImageCredit}}" style="float: left; height: 80px; margin-right: 20px; margin-bottom: 10px;"> ooo
    <h3>{{ event.Date | date: "%a %-d %B %Y" }} - {{ event.Times }}</h3>
    <h4>{{ event.EventTitle }}</h4> aaa
     
    {{ event.LongDesc }}
     b   
    <b>Venue:</b> {{ event.VenueAddress }}<br/>
    <b>Phone:</b> {{ event.VenuePhone }}<br/>
    <b>Website:</b> <a href="https://{{ event.VenueWebsite }}" target="_blank">Click here</a><br/>
